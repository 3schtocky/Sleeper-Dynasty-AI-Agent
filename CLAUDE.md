...
Phase 6 (season-outcome simulation) is built and live-verified: `simulate-season` runs a Monte Carlo simulation of the league's real remaining schedule (real Sleeper matchup pairings, confirmed live to already exist for every future regular-season week), reporting playoff odds, average final wins/rank, and last-place odds per team, using the Phase 5 calibration when one is fitted.

Phase 7 (install/distribution overhaul) is built and live-verified: `install.sh`/`install.ps1` take a fresh machine to a real global `dynasty-agent` command plus a running local LLM server (Qwen3-4B via LM Studio, headless, no GUI app needed) in one shot. The full sequence, including a real tool-calling round trip against the local model, was run for real, not just documented; see `PLANNING.md` for the exact commands and output, including a real correction to the originally planned model identifier. `PLANNING.md` is the build log, every methodology decision, real bug found and fixed, and live-verification result lives there; this file stays the reference doc, read `PLANNING.md` for the "why" behind a specific number.
...
- **Phase 7, install/distribution overhaul. Done.** `install.sh`/`install.ps1` install `uv`, LM Studio (headless, no GUI app, via LM Studio's own official installer), the local model (`qwen/qwen3-4b`, confirmed the real working identifier live), and `dynasty-agent` itself as a real global command (`uv tool install`), then walk through `dynasty-agent init`. Verified live end to end in this project's own dev session, including a real tool-calling round trip through the local model, ahead of Phase 8. See `PLANNING.md`.
