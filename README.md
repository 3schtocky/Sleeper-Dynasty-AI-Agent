...
- **`simulate-season`** — Monte Carlo simulation of your league's real remaining schedule (real Sleeper matchup pairings, not a guessed one): playoff odds, average final wins and rank, and last-place odds per team, using the calibrated matchup model once you've fitted one.
